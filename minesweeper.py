...
class Minesweeper():
    ...
    def is_mine(self, move):
        i, j = move
        if 0 <= i < self.height and 0 <= j < self.width:
            return self.board[i][j]
        else:
            return False  # move koordinatları oyun tahtasının dışında ise False döndür
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                if (i, j) in self.mines:
                    countMines += 1
                if  0 <= i <= self.height and  0 <= j <= self.height and (i, j) not in self.safes and (i, j) not in self.mines:
